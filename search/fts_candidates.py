...
async def fetch_fts_candidates(  # pragma: no cover - phụ thuộc Postgres thật
    ...
    *,
    ...
    `filters` (Story 2.3, AD-21) áp dụng trong subquery `limited`, TRƯỚC `ORDER BY`/`LIMIT` —
    GIỮ NGUYÊN kỷ luật subquery-trước-`ts_headline` từ code review Story 2.2 (filter KHÔNG
    được áp ở tầng ngoài, nơi `ts_headline` đã tính trên `pool_size` dòng đã cắt).
    """
    ...
    limited = (
        apply_scene_filters(limited_stmt, filters)
